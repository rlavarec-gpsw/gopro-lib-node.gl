...
#include <backends/d3d12/impl/D3DBlitOp.h>
...
void D3DTexture::createDepthStencilView()
{
	...
	dsvDescriptor = std::make_unique<D3DDescriptorHandle>();
	dsvDescriptorHeap->getHandle(*dsvDescriptor);
	D3D_TRACE(d3dDevice->CreateDepthStencilView(
		mID3D12Resource.Get(), &dsvDesc, dsvDescriptor->cpuHandle));
}

void D3DTexture::init(D3DGraphicsContext* ctx, D3DGraphics* graphics, void* data,
						...
						uint32_t numSamples, const D3DSamplerDesc* samplerDesc, int32_t dataPitch)
{
	...
	mCurrentResourceState.resize(numSubresources);
...
D3DTexture* D3DTexture::newInstance(D3DGraphicsContext* graphicsContext, D3DGraphics* graphics, void* data,
...
{
	// TODO Renan check validity
	mID3D12Resource = (ID3D12Resource*)handle;

	for(auto& s : mCurrentResourceState)
		s = D3D12_RESOURCE_STATE_COMMON;

	auto commandList = ctx->drawCommandBuffer(ctx->currentImageIndex);
...
D3DSampler* D3DTexture::getSampler(D3D12_FILTER filter)
{
	...
	auto sampler = std::make_unique<D3DSampler>();
	...
	uint32_t numMipLevels, uint32_t plane)
{
	...
	auto d3dDevice = ctx->d3dDevice.mID3D12Device.Get();
	auto handle = std::make_unique<D3DDescriptorHandle>();
	cbvSrvUavDescriptorHeap.getHandle(*handle);
	D3D_TRACE(d3dDevice->CreateShaderResourceView(
		mID3D12Resource.Get(), &srvDesc, handle->cpuHandle));
	SrvData srvData;
...
D3DDescriptorHandle* D3DTexture::getUavDescriptor(uint32_t mipLevel, uint32_t plane)
{
	...
	D3D_TRACE(d3dDevice->CreateUnorderedAccessView(
		mID3D12Resource.Get(), nullptr, &uavDesc, handle->cpuHandle));
	UavData uavData;
	...
	uint32_t planeIndex)
{
	...
	D3D_TRACE(d3dDevice->CreateRenderTargetView(
		mID3D12Resource.Get(), &rtvDesc, handle->cpuHandle));
	RtvData rtvData;
	...
	int32_t arrayLayers, int32_t numPlanes, int32_t dataPitch)
{
	...
	if(data && mipLevels != 1)
	{
		...
		cmdList.end();
		ctx->d3dCommandQueue.submit(cmdList.mGraphicsCommandList.Get(), nullptr);
		ctx->d3dCommandQueue.waitIdle();
	}
}

void D3DTexture::generateMipmaps(D3DCommandList* commandBuffer)
{
	generateMipmapsFn(commandBuffer);
}
	...
	int32_t, int32_t, int32_t dataPitch)
{
	if(data)
	{
		...
		std::vector<D3D12_SUBRESOURCE_DATA> textureData(arrayLayers * numPlanes);
	...
	int32_t arrayLayers, int32_t numPlanes)
{
	...
	if(numPlanes == -1)
		numPlanes = this->numPlanes;
	D3D12_RESOURCE_DESC desc = mID3D12Resource->GetDesc();
	...
	for(int j = 0; j < subresourceData.size(); j++)
	{
		...
		D3D_TRACE(ctx->d3dDevice.mID3D12Device->GetCopyableFootprints(
	...
	int subresourceIndex)
{
	...
	D3D12_TEXTURE_COPY_LOCATION dstLocation = {
		readbackBuffer.mID3D12Resource.Get(),
		D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT,
		{dstFootprint} };
	D3D12_TEXTURE_COPY_LOCATION srcLocation = {
		mID3D12Resource.Get(), D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX, UINT64(subresourceIndex) };
...
void D3DTexture::changeLayout(D3DCommandList* commandBuffer,
	ImageLayout imageLayout)
{
	...
	switch(imageLayout)
	{
		...
		case IMAGE_LAYOUT_UNORDERED_ACCESS:
			...
			break;
		default:
			NGLI_ERR("imageLayout: %d", imageLayout);
	};
	resourceBarrierTransition(commandBuffer, resourceState);
}

template<typename T>
static bool allEqual(const std::vector<T>& resourceState)
{
	if(resourceState.empty())
		return false;
	auto v0 = resourceState[0];
	for(int j = 1; j < resourceState.size(); j++)
		if(resourceState[j] != v0)
			return false;
	return true;
}
	...
	UINT subresource)
{
	...
	{
		if(mCurrentResourceState[0] != newState && allEqual(mCurrentResourceState))
		{
			auto resourceBarrier = CD3DX12_RESOURCE_BARRIER::Transition(mID3D12Resource.Get(), mCurrentResourceState[0],
				newState, D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES);
			D3D_TRACE(cmdList->mGraphicsCommandList->ResourceBarrier(1, &resourceBarrier));
			std::fill(mCurrentResourceState.begin(), mCurrentResourceState.end(), newState);
			return;
		}
		j0 = 0;
		j1 = numSubresources;
	...
	{
		if(mCurrentResourceState[j] == newState)
		{
			continue;
		}
		resourceBarriers.push_back(
			CD3DX12_RESOURCE_BARRIER::Transition(mID3D12Resource.Get(), mCurrentResourceState[j],
												 newState, j)
		);
		mCurrentResourceState[j] = newState;
	}
	if(!resourceBarriers.empty())
		D3D_TRACE(cmdList->mGraphicsCommandList->ResourceBarrier(resourceBarriers.size(), resourceBarriers.data()));
}
