...
static void vk_transition_image_layout(VkCommandBuffer cmd_buf,
                                       VkImage image,
...
VkResult ngli_texture_vk_init(struct texture *s, const struct texture_params *params)
{
    ...
    const VkImageCreateInfo image_create_info = {
        ...
        .usage         = usage,
        .samples       = ngli_ngl_samples_to_vk(params->samples),
    ...
    int32_t lazy_allocated = 0;
    int32_t memory_type_index = NGL_ERROR_NOT_FOUND;
    const int is_transient_attachment = params->usage & NGLI_TEXTURE_USAGE_TRANSIENT_ATTACHMENT_BIT;
    if (is_transient_attachment) {
        const VkMemoryPropertyFlags memory_property_flags = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT |
                                                            VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT;
        memory_type_index = ngli_vkcontext_find_memory_type(vk, requirements.memoryTypeBits, memory_property_flags);
        lazy_allocated = memory_type_index >= 0;
    }
    ...
    const VkMemoryAllocateInfo alloc_info = {
        ...
        .allocationSize  = lazy_allocated ? 0 : requirements.size,
...
VkResult ngli_texture_vk_wrap(struct texture *s, const struct texture_params *params, VkImage image, VkImageLayout layout)
{
    ...
    s_priv->default_image_layout = VK_IMAGE_LAYOUT_GENERAL;
    if (params->usage & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT)
        s_priv->default_image_layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    if (params->usage & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT)
        s_priv->default_image_layout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    if (params->usage & VK_IMAGE_USAGE_SAMPLED_BIT)
        s_priv->default_image_layout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    if (params->usage & VK_IMAGE_USAGE_STORAGE_BIT)
        s_priv->default_image_layout = VK_IMAGE_LAYOUT_GENERAL;
...
void ngli_texture_vk_transition_layout(struct texture *s, VkImageLayout layout)
{
    ...
    vk_transition_image_layout(cmd_buf, s_priv->image, s_priv->image_layout, layout, &subres_range);
...
VkResult ngli_texture_vk_upload(struct texture *s, const uint8_t *data, int linesize)
{
    ...
    if (!s_priv->staging_buffer || s_priv->staging_buffer_row_length != linesize) {
        ...
        const int staging_buffer_size = width * s->params.height * s->params.depth * bytes_per_pixel * s_priv->array_layers;
    ...
    vk_transition_image_layout(cmd_buf,
                               s_priv->image,
                               s_priv->image_layout,
