...
void ngli_rendertarget_vk_read_pixels(struct rendertarget *s, uint8_t *data)
{
    ...
    VkCommandBuffer command_buffer = gpu_ctx_vk->cur_command_buffer;
    ...
    if (res != VK_SUCCESS)
        return;
    gpu_ctx_vk->cur_command_buffer_state = 0;

    const VkSubmitInfo submit_info = {
        .sType                = VK_STRUCTURE_TYPE_SUBMIT_INFO,
        .waitSemaphoreCount   = ngli_darray_count(&gpu_ctx_vk->wait_semaphores),
        .pWaitSemaphores      = ngli_darray_data(&gpu_ctx_vk->wait_semaphores),
        .pWaitDstStageMask    = ngli_darray_data(&gpu_ctx_vk->wait_stages),
        .commandBufferCount   = 1,
        .pCommandBuffers      = &command_buffer,
        .signalSemaphoreCount = 0,
        .pSignalSemaphores    = NULL,
    };
    ngli_darray_clear(&gpu_ctx_vk->wait_semaphores);
    ngli_darray_clear(&gpu_ctx_vk->wait_stages);

    res = vkQueueSubmit(vk->graphic_queue, 1, &submit_info, gpu_ctx_vk->fences[gpu_ctx_vk->frame_index]);
    if (res != VK_SUCCESS)
        return;

    res = vkWaitForFences(vk->device, 1, &gpu_ctx_vk->fences[gpu_ctx_vk->frame_index], VK_TRUE, UINT64_MAX);
    if (res != VK_SUCCESS)
        return;

    res = vkResetFences(vk->device, 1, &gpu_ctx_vk->fences[gpu_ctx_vk->frame_index]);
    if (res != VK_SUCCESS)
        return;
    ...
    res = vkBeginCommandBuffer(gpu_ctx_vk->cur_command_buffer, &command_buffer_begin_info);
    if (res != VK_SUCCESS)
        return;
    gpu_ctx_vk->cur_command_buffer_state = 1;
}
