...
static VkResult create_dummy_texture(struct gpu_ctx *s)
{
    ...
    uint8_t buf[4] = {0};
    res = ngli_texture_vk_upload(s_priv->dummy_texture, buf, 0);
    if (res != VK_SUCCESS)
        return res;
...
static VkResult create_command_pool_and_buffers(struct gpu_ctx *s)
{
    ...
    s_priv->cmd_bufs = ngli_calloc(s_priv->nb_in_flight_frames, sizeof(VkCommandBuffer));
    if (!s_priv->cmd_bufs)
        return VK_ERROR_OUT_OF_HOST_MEMORY;
...
static void destroy_command_pool_and_buffers(struct gpu_ctx *s)
{
    ...
    if (s_priv->cmd_bufs) {
        vkFreeCommandBuffers(vk->device, s_priv->cmd_pool,
                             s_priv->nb_in_flight_frames, s_priv->cmd_bufs);
        ngli_freep(&s_priv->cmd_bufs);
    }
...
static VkResult recreate_swapchain(struct gpu_ctx *gpu_ctx, struct vkcontext *vk)
{
    ...
    /*
     * According to the Vulkan specification, on Windows, the window size maybe
...
static VkResult swapchain_present_buffer(struct gpu_ctx *s)
{
    ...
    const VkPresentInfoKHR present_info = {
        ...
        .waitSemaphoreCount = ngli_darray_count(&s_priv->signal_sems),
...
static int vk_init(struct gpu_ctx *s)
{
    ...
    ngli_darray_init(&s_priv->wait_sems, sizeof(VkSemaphore), 0);
    ngli_darray_init(&s_priv->wait_stages, sizeof(VkPipelineStageFlagBits), 0);
    ngli_darray_init(&s_priv->signal_sems, sizeof(VkSemaphore), 0);
...
static int vk_begin_draw(struct gpu_ctx *s, double t)
{
    ...
    if (config->hud) {
        ...
        vkCmdWriteTimestamp(s_priv->cur_cmd_buf, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, s_priv->query_pool, 0);
    }
...
static int vk_query_draw_time(struct gpu_ctx *s, int64_t *time)
{
    ...
    ngli_assert(s_priv->cur_cmd_buf);
...
static VkResult vk_submit_cmd_buf(struct gpu_ctx *s)
{
    struct gpu_ctx_vk *s_priv = (struct gpu_ctx_vk *)s;
    struct vkcontext *vk = s_priv->vkcontext;
...
static void vk_destroy(struct gpu_ctx *s)
{
    ...
    /* FIXME: what to do if vkDeviceWaitIdle() fails, ignore graphic call with
     * device lost, and try continue in case of mem error? */
    VkResult res = vkDeviceWaitIdle(vk->device);
    if (res != VK_SUCCESS)
        return;
    ...
    ngli_darray_reset(&s_priv->wait_sems);
    ngli_darray_reset(&s_priv->wait_stages);
    ngli_darray_reset(&s_priv->signal_sems);
