...
static VkResult pipeline_graphics_init(struct pipeline *s, const struct pipeline_params *params)
{
    ...
    const VkViewport viewport = {
        .width    = gpu_ctx_vk->width,
        .height   = gpu_ctx_vk->height,
        .minDepth = 0.f,
        .maxDepth = 1.f,
    };

    const VkRect2D scissor = {
        .extent = gpu_ctx_vk->extent,
    };
...
int ngli_pipeline_vk_set_resources(struct pipeline *s, const struct pipeline_resource_params *params)
{
    ...
    for (int i = 0; i < params->nb_buffers; i++) {
        ...
        int ret = ngli_pipeline_vk_update_buffer(s, i, params->buffers[i], buffer_desc->offset, buffer_desc->size);
        if (ret < 0)
            return ret;
    }
...
int ngli_pipeline_vk_update_texture(struct pipeline *s, int index, struct texture *texture)
{
    ...
    const struct texture_vk *texture_vk = (struct texture_vk *)(texture_binding->texture);
...
void ngli_pipeline_vk_draw(struct pipeline *s, int nb_vertices, int nb_instances)
{
    ...
    VkCommandBuffer cmd_buf = gpu_ctx_vk->cur_command_buffer;
    ...
    VkRect2D scissor;
    const struct rendertarget *rt = gpu_ctx_vk->rendertarget;
    const struct pipeline_graphics *graphics = &s->graphics;
