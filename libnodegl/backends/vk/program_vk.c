...
int ngli_program_vk_init(struct program *s, const char *vertex, const char *fragment, const char *compute)
{
    ...
    for (int i = 0; i < NGLI_ARRAY_NB(s_priv->shaders); i++) {
        ...
        shaderc_compilation_result_t result = shaderc_compile_into_spv(gpu_ctx_vk->spirv_compiler,
                                                                       shaders[i].src, strlen(shaders[i].src),
                                                                       shaders[i].kind,
                                                                       "program", // XXX: use node label?
                                                                       "main",
                                                                       gpu_ctx_vk->spirv_compiler_opts);

        shaderc_compilation_status status = shaderc_result_get_compilation_status(result);
        if (status != shaderc_compilation_status_success) {
            LOG(ERROR, "failed to compile shader: %s", shaderc_result_get_error_message(result));
