...
RenderPass* get_render_pass(GraphicsContext* ctx, const struct rendertarget_params* params) {
    ...
    if (hasDepthStencilAttachment) rp_config.depthStencilAttachmentDescription = {
        to_ngfx_format(params->depth_stencil.attachment->params.format), nullopt, nullopt,
        to_ngfx_load_op(params->depth_stencil.load_op), to_ngfx_store_op(params->depth_stencil.store_op)
    };
