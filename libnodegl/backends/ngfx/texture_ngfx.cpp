...
int ngli_texture_ngfx_init(struct texture *s, const struct texture_params *p)
{
    ...
    samplerDesc.mipFilter    = gen_mipmaps
                                   ? to_ngfx_mip_filter_mode(p->mipmap_filter)
                                   : FILTER_NEAREST;
    samplerDesc.addressModeU = CLAMP_TO_EDGE;
    samplerDesc.addressModeV = CLAMP_TO_EDGE;
    samplerDesc.addressModeW = CLAMP_TO_EDGE;

    s_priv->v = Texture::create(
        gpu_ctx, ctx->graphics, nullptr, to_ngfx_format(p->format), size,
        p->width, p->height, depth, array_layers, image_usage_flags,
        to_ngfx_texture_type(p->type), gen_mipmaps,
        p->samples == 0 ? 1 : p->samples, &samplerDesc);

    return 0;
}

int ngli_texture_ngfx_upload(struct texture *s, const uint8_t *data,
                             int linesize)
{
