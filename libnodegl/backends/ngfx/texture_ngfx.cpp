...
int ngli_texture_ngfx_init(struct texture *s,
    ...
    uint32_t size = s_priv->bytes_per_pixel * p->width * p->height * depth * array_layers;
    SamplerDesc samplerDesc;
    samplerDesc.minFilter = to_ngfx_filter_mode(p->min_filter);
    samplerDesc.magFilter = to_ngfx_filter_mode(p->mag_filter);
    samplerDesc.mipFilter = gen_mipmaps ? to_ngfx_mip_filter_mode(p->mipmap_filter) : FILTER_NEAREST;
    samplerDesc.addressModeU = CLAMP_TO_EDGE;
    samplerDesc.addressModeV = CLAMP_TO_EDGE;
    samplerDesc.addressModeW = CLAMP_TO_EDGE;
...
int ngli_texture_ngfx_upload(struct texture *s, const uint8_t *data, int linesize) {
    ...
    uint32_t size = texture->bytes_per_pixel * linesize * s->params.height * s->params.depth * texture->v->arrayLayers;
    texture->v->upload((void *)data, size, 0, 0, 0, s->params.width, s->params.height, s->params.depth, texture->v->arrayLayers, -1, texture->bytes_per_pixel * linesize);
    return 0;
}
