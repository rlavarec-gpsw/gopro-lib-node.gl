...
struct rendertarget *ngli_rendertarget_ngfx_create(struct gpu_ctx *gpu_ctx)
{
...
int ngli_rendertarget_ngfx_init(struct rendertarget *s,
                                const struct rendertarget_params *params)
{
    ...
    for (int i = 0; i < params->nb_colors; i++) {
        ...
        const texture_ngfx *color_texture =
            (const texture_ngfx *)color_attachment->attachment;
        const texture_ngfx *resolve_texture =
            (const texture_ngfx *)color_attachment->resolve_target;
    ...
    const texture_ngfx *depth_texture =
        (const texture_ngfx *)depth_attachment->attachment;
    if (depth_texture) {
        const texture_ngfx *resolve_texture =
            (const texture_ngfx *)depth_attachment->resolve_target;
    ...
    s_priv->output_framebuffer = Framebuffer::create(
