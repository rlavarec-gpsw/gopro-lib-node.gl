...
int ngli_buffer_ngfx_init(struct buffer *s, int size, int usage)
{
    ...
    // etc
    s_priv->v = Buffer::create(
        ctx->graphics_context, NULL, size,
        BUFFER_USAGE_VERTEX_BUFFER_BIT | BUFFER_USAGE_INDEX_BUFFER_BIT |
            BUFFER_USAGE_UNIFORM_BUFFER_BIT | BUFFER_USAGE_STORAGE_BUFFER_BIT);
    return 0;
}
...
int ngli_buffer_ngfx_map(struct buffer *s, int size, int offset, void **data)
{
    struct buffer_ngfx *s_priv = (struct buffer_ngfx *)s;
    s->data                    = s_priv->v->map();
    *data                      = s->data;
