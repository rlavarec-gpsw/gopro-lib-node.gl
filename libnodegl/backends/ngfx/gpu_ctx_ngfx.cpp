...
{
    struct gpu_ctx_ngfx* s_priv = (gpu_ctx_ngfx*)s;

    ngli_texture_freep(&s_priv->dummy_texture);
}

static int create_texture(gpu_ctx* s, int format, int width, int height, int samples, int usage, texture** texturep)
{
...
static int create_rendertarget(struct gpu_ctx* s, texture* color, texture* resolve_color, texture* depth_stencil,
        int color_load_op, int color_store_op, int depth_stencil_load_op, int depth_stencil_store_op, rendertarget** rendertargetp)
{
...
static int create_offscreen_resources(struct gpu_ctx *s) {
    ...
    int usage = NGLI_TEXTURE_USAGE_COLOR_ATTACHMENT_BIT | NGLI_TEXTURE_USAGE_TRANSFER_SRC_BIT;
    if (config->samples == 1)
        usage |= NGLI_TEXTURE_USAGE_SAMPLED_BIT;
    auto& color_texture = s_priv->offscreen_resources.color_texture;
    int res = create_texture(s, NGLI_FORMAT_R8G8B8A8_UNORM, config->width, config->height, config->samples, usage,
        &color_texture);
    if (res < 0)
        return res;
    ...
    if (config->samples) {
        res = create_texture(s, NGLI_FORMAT_R8G8B8A8_UNORM, config->width, config->height, 1,
            NGLI_TEXTURE_USAGE_COLOR_ATTACHMENT_BIT | NGLI_TEXTURE_USAGE_TRANSFER_SRC_BIT | NGLI_TEXTURE_USAGE_SAMPLED_BIT,
...
extern "C" const struct gpu_ctx_class ngli_gpu_ctx_ngfx = {
    ...
    .pipeline_update_buffer     = ngli_pipeline_ngfx_update_buffer,
