...
#endif
    if (config->offscreen) {
        ctx->surface = surface_util_ngfx::create_offscreen_surface(
            config->width, config->height);
    } else {
        ctx->surface = surface_util_ngfx::create_surface_from_window_handle(
            graphics_context, config->platform, config->display, config->window,
            config->width, config->height);
        ctx->swapchain_util =
            swapchain_util_ngfx::create(graphics_context, config->window);
    }
...
static int ngfx_resize(struct gpu_ctx *s, int width, int height,
                       const int *viewport)
{
    const ngl_config *config = &s->config;
    if (config->offscreen) {
...
static int ngfx_begin_update(struct gpu_ctx *s, double t) { return 0; }

static int ngfx_end_update(struct gpu_ctx *s, double t) { return 0; }
...
static int ngfx_end_draw(struct gpu_ctx *s, double t)
{
    ...
    if (s->config.offscreen) {
        ...
        if (s->config.capture_buffer) {
            ...
            auto &output_texture =
                ((texture_ngfx *)(output_color_resolve_texture
...
ngfx_get_default_rendertarget_desc(struct gpu_ctx *s)
{
    struct gpu_ctx_ngfx *s_priv = (struct gpu_ctx_ngfx *)s;
    return &s_priv->default_rendertarget_desc;
}

static void begin_render_pass(struct gpu_ctx_ngfx *s_priv,
                              rendertarget_ngfx *rt_priv)
{
...
extern "C" const struct gpu_ctx_class ngli_gpu_ctx_ngfx = {
    ...
    .get_preferred_depth_stencil_format =
        ngfx_get_preferred_depth_stencil_format,
