...
using VertexInputAttributeDescription = GraphicsPipeline::VertexInputAttributeDescription;
...
static int pipeline_set_uniforms(pipeline_ngfx* s)
{
    ...
    for (int i = 0; i < nb_buffers; i++) {
        const buffer_binding* binding = (buffer_binding*)ngli_darray_get(&s->buffer_bindings, i);
        const struct buffer* buffer = (struct buffer*)binding->buffer;
        int ret = ngli_buffer_upload((struct buffer*)buffer, buffer->data, buffer->size, 0);
        if (ret < 0)
            return ret;
    }
    return 0;
...
static std::vector<VertexInputAttributeDescription> get_vertex_attributes(VertexShaderModule* vs, const pipeline_params* params) {
...
static int pipeline_graphics_init(pipeline *s, const pipeline_params* params)
{
...
#if defined(NGFX_GRAPHICS_BACKEND_VULKAN)
    state.renderPass = get_compat_render_pass(gpu_ctx->graphics_context, params->graphics.rt_desc);
#endif
...
#if defined(NGFX_GRAPHICS_BACKEND_DIRECT3D12) || defined(NGFX_GRAPHICS_BACKEND_VULKAN)
    ...
    s_priv->gp = GraphicsPipeline::create(
        ...
        get_instance_attributes(layout->attributes_desc, layout->nb_attributes)
    );
...
int ngli_pipeline_ngfx_set_resources(pipeline *s, const pipeline_resources *resources)
{
    pipeline_ngfx *s_priv = (pipeline_ngfx *)s;

    ngli_assert(ngli_darray_count(&s_priv->attribute_bindings) == resources->nb_attributes);
    for (int i = 0; i < resources->nb_attributes; i++) {
        int ret = ngli_pipeline_ngfx_update_attribute(s, i, resources->attributes[i]);
        if (ret < 0)
            return ret;
    }
    ...
    for (int i = 0; i < resources->nb_buffers; i++) {
        const buffer_binding* buffer_binding_data = (const buffer_binding*)ngli_darray_get(&s_priv->buffer_bindings, i);
        const pipeline_buffer_desc* buffer_desc = &buffer_binding_data->desc;
        int ret = ngli_pipeline_ngfx_update_buffer(s, i, resources->buffers[i], buffer_desc->offset, buffer_desc->size);
        if (ret < 0)
            return ret;
    }
    ...
    for (int i = 0; i < resources->nb_textures; i++) {
        int ret = ngli_pipeline_ngfx_update_texture(s, i, resources->textures[i]);
        if (ret < 0)
            return ret;
    }
...
int ngli_pipeline_ngfx_update_attribute(pipeline *s, int index, const buffer *p_buffer) {
    pipeline_ngfx *s_priv = (pipeline_ngfx *)s;
...
int ngli_pipeline_ngfx_update_texture(pipeline *s, int index, const texture *p_texture) {
    gpu_ctx_ngfx *gpu_ctx = (gpu_ctx_ngfx *)s->gpu_ctx;
    pipeline_ngfx *s_priv = (pipeline_ngfx *)s;
...
int ngli_pipeline_ngfx_update_buffer(pipeline *s, int index, const buffer *p_buffer, int offset, int size)
{
...
static int get_binding(pipeline_ngfx* s_priv, int set) {
    return s_priv->gp ?
        s_priv->gp->descriptorBindings[set] :
...
static void bind_buffers(CommandBuffer *cmd_buf, pipeline *s) {
    ...
    for (int j = 0; j<nb_buffers; j++) {
        ...
        if (buffer_desc.type == NGLI_TYPE_UNIFORM_BUFFER) {
            ...
            if (!buffer_info)
                continue; //unused variable
            gpu_ctx->graphics->bindUniformBuffer(cmd_buf, buffer->v, get_binding(s_priv, buffer_info->set), buffer_info->shaderStages);
        }
        else {
            auto buffer_info = shader_module->findStorageBufferInfo(buffer_desc.name);
            if (!buffer_info)
                continue; //unused variable
            gpu_ctx->graphics->bindStorageBuffer(cmd_buf, buffer->v, get_binding(s_priv, buffer_info->set), buffer_info->shaderStages,
...
static void bind_textures(CommandBuffer *cmd_buf, pipeline *s) {
    ...
    for (int j = 0; j<nb_textures; j++) {
        ...
        if (texture) {
            gpu_ctx->graphics->bindTexture(cmd_buf, texture->v, get_binding(s_priv, texture_info->set));
        }
    }
}
...
void ngli_pipeline_ngfx_draw_indexed(pipeline *s, const buffer *indices, int indices_format, int nb_indices, int nb_instances) {
    gpu_ctx_ngfx *gpu_ctx = (gpu_ctx_ngfx *)s->gpu_ctx;
    CommandBuffer *cmd_buf = gpu_ctx->cur_command_buffer;
