...
int ngli_program_ngfx_init(struct program *s, const program_params* p) {
    ...
    try {
        if (p->vertex) {
            ShaderCompiler sc;
            program->vs = VertexShaderModule::create(gpu_ctx->graphics_context->device, sc.compile(p->vertex, ".vert")).release();
        }
        if (p->fragment) {
            ShaderCompiler sc;
            program->fs = FragmentShaderModule::create(gpu_ctx->graphics_context->device, sc.compile(p->fragment, ".frag")).release();
        }
        if (p->compute) {
            ShaderCompiler sc;
            program->cs = ComputeShaderModule::create(gpu_ctx->graphics_context->device, sc.compile(p->compute, ".comp")).release();
        }
    }
