...
static char get_ngfx_backend_id() {
...
struct ShaderCompiler {
    ...
    {
        size_t h1 = std::hash<string>{}(get_ngfx_backend_id() + src);
        tmpDir      = fs::path(FileUtil::tempDir() + "/" + "nodegl")
                     .make_preferred()
                     .string();
        fs::create_directories(tmpDir);
        string tmpFile = fs::path(tmpDir + "/" + "tmp_" + to_string(h1) + ext)
                             .make_preferred()
                             .string();
    ...
    defined(NGFX_GRAPHICS_BACKEND_DIRECT3D12)
        flags |= ShaderTools::FLIP_VERT_Y;
#endif
        spvFiles = shaderTools.compileShaders(
            glslFiles, outDir, ShaderTools::FORMAT_GLSL, {}, flags);
#if defined(NGFX_GRAPHICS_BACKEND_VULKAN)
        glslMapFiles = shaderTools.generateShaderMaps(glslFiles, outDir,
                                                      ShaderTools::FORMAT_GLSL);
#elif defined(NGFX_GRAPHICS_BACKEND_DIRECT3D12)
        hlslFiles    = shaderTools.convertShaders(spvFiles, outDir,
        ...
        hlslMapFiles = shaderTools.generateShaderMaps(hlslFiles, outDir,
                                                      ShaderTools::FORMAT_HLSL);
#elif defined(NGFX_GRAPHICS_BACKEND_METAL)
        mtlFiles    = shaderTools.convertShaders(spvFiles, outDir,
                                                 ShaderTools::FORMAT_MSL);
        mtllibFiles = shaderTools.compileShaders(mtlFiles, outDir,
                                                 ShaderTools::FORMAT_MSL);
        mtlMapFiles = shaderTools.generateShaderMaps(mtlFiles, outDir,
                                                     ShaderTools::FORMAT_MSL);
#endif
...
int ngli_program_ngfx_init(struct program *s, const program_params *p)
{
    ...
    try {
        if (p->vertex) {
            ShaderCompiler sc;
            program->vs =
                VertexShaderModule::create(gpu_ctx->graphics_context->device,
        ...
        if (p->fragment) {
            ShaderCompiler sc;
            program->fs =
                FragmentShaderModule::create(gpu_ctx->graphics_context->device,
        ...
        if (p->compute) {
            ShaderCompiler sc;
            program->cs =
                ComputeShaderModule::create(gpu_ctx->graphics_context->device,
