...
{
    ngli_gpu_ctx_gl_reset_state(s->gpu_ctx);
    int ret = ngli_ctx_set_capture_buffer(s, capture_buffer);
    ngli_gpu_ctx_gl_reset_state(s->gpu_ctx);
    return ret;
}
...
{
    LOG(ERROR, "context does not support wrapping external OpenGL framebuffers");
...
static int glw_wrap_framebuffer(struct ngl_ctx *s, uint32_t framebuffer)
{
    ...
    config_gl->framebuffer = framebuffer;
...
{
    struct ngl_config_gl *config_gl = config->backend_config;
    return config_gl && config_gl->wrapped;
}
