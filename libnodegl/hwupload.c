...
static const struct hwmap_class *hwmap_classes[] = {
#ifdef BACKEND_GL
...
{
    for (int i = 0; i < NGLI_ARRAY_NB(hwmap_classes); i++) {
        const struct hwmap_class *hwmap_class = hwmap_classes[i];
        if (hwmap_class &&
            hwmap_class->hwformat == frame->pix_fmt &&
           (hwmap_class->backends & HWMAP_BACKEND(backend)))
            return hwmap_class;
    }
