...
def _nodegl_setup(cfg):
    nodegl_opts = ["--default-library", "shared"]
    ...
    if cfg.args.enable_ngfx_backend:
        nodegl_opts += [f"-Dgbackend-ngfx=enabled"]
        if _SYSTEM == "Darwin":
            ...
            nodegl_opts += [f"-Dgbackend-vk=disabled"]
        nodegl_opts += [f"-Dngfx_graphics_backend=$(NGFX_GRAPHICS_BACKEND)"]
    else:
        nodegl_opts += [f"-Dgbackend-ngfx=disabled"]
